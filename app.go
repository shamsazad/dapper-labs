...
func (a *App) HandleRequests() {
	...
	authenticatedRouter.HandleFunc("/dapper-lab/update/user", handlers.UpdateUser(app.Repo)).Methods(http.MethodPost)
	authenticatedRouter.HandleFunc("/dapper-lab/users", handlers.GetAllUsers(app.Repo)).Methods(http.MethodGet)
	app.Router.HandleFunc("/dapper-lab/user", handlers.SignUp(app.Repo)).Methods(http.MethodPost)
	app.Router.HandleFunc("/dapper-lab/login", handlers.Login(app.Repo)).Methods(http.MethodPost)
